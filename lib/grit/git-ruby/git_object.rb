...
module Grit
  ...
  class Tree < GitObject
    ...
    def raw_content
      ...
      @entry.collect { |e|
          # Just for tree object, use e.name instead of e.safe_name
          # because sometimes a filename from osx may be ended with "\r"
          [[e.format_mode, e.format_type, e.sha1].join(' '), e.name.end_with?("\r") ? e.name : e.safe_name].join("\t")
          }.join("\n")
    end
